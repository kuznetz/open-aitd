...
namespace openAITD {
    ...
    float brightnessTrg = 1;
      ...
      //view matrix
		...
		Vector3 WorldToScreenZ(Vector3 position);
	};
	...
	Vector3 World::WorldToScreenZ(Vector3 position) {
			// Get screen dimensions (in pixels)
			const int width  = resources->config.screenW;
			const int height = resources->config.screenH;

			// Retrieve the view and projection matrices from the camera
			const Matrix& view = this->cameraView;
			const Matrix& proj = this->cameraProjection;

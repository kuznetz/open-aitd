// Based on UnPAK, By Cyril VOILA (cvoila@free.fr)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "pak.h"
...
namespace AITDExtractor {
  ...
  // --------------------------------------------------------------
  const int PAK_BMAX = 16;
  const int PAK_N_MAX = 288;
  const int PAK_WSIZE = 0x8000;
  ...
  int PakFile::PAK_explode(unsigned char * srcBuffer, unsigned char * dstBuffer, unsigned int compressedSize, unsigned int uncompressedSize, unsigned short flags)
  {
  ...
  int PakFile::PAK_deflate(unsigned char * srcBuffer, unsigned char * dstBuffer, unsigned int compressedSize, unsigned int uncompressedSize) {
    z_stream G;
    G.next_in = srcBuffer;

...
namespace triangulate
{
	...
	struct TriangleIdx
	{
		TriangleIdx(const int& p0, const int& p1, const int& p2) : p0(p0), p1(p1), p2(p2) {}
		TriangleIdx(const TriangleIdx& t) : p0(t.p0), p1(t.p1), p2(t.p2) {}
		int p0, p1, p2;
	};
	...
	template <typename T = Point>
	int sizePolygon(const std::vector<T>& polygon)
	{
	...
	template <typename T = Point>
	bool isEar(const int index, const std::vector<T>& polygon, const T& normal)
	{
		const auto n = sizePolygon(polygon);
	...
	std::vector<TriangleIdx> fanTriangulation(std::vector<P>& polygon)
	{
		...
		for( int index = 1; index < n - 1; ++index )
			triangles.emplace_back(0, index, index + 1);

		return triangles;
	}

	template <typename P = Point, typename T = Triangle<P>>
	std::vector<TriangleIdx> cutTriangulation(std::vector<P>& polygon, const P& normal)
	{
		std::vector<TriangleIdx> triangles;
		...
		return polygon.size() == 2 ? triangles : std::vector<TriangleIdx>();
	}

	template <typename P = Point, typename T = Triangle<P>>
	std::vector<TriangleIdx> triangulate(const std::vector<P>& polygon)
	{
